...
import { boards, widgetData, widgetInstances } from "@/db/schema";

// No "server-only" here: this module is imported both from server-only app
// code (lib/widget-queries.ts, which auto-seeds a new user's boards on
// first visit) and from db/seed.ts, a standalone CLI script that can't
// import anything server-only-tagged. Parametrized by `db` instead of
// importing the server-only db/client.ts singleton directly, so each
// caller can pass in whichever client it's allowed to build.
